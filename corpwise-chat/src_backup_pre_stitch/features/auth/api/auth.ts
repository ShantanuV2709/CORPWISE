...
export async function adminSignup(username: string, password: string, company_id: string, email: string, subscription_tier: string = "starter") {
    const res = await fetch(`${API_BASE}/auth/admin/signup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, password, company_id, email, subscription_tier }),
    });
...
export async function googleAdminLogin(token: string, company_id?: string) {
    const res = await fetch(`${API_BASE}/auth/google`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, company_id, role: "admin" }),
    });

    if (!res.ok) {
        const err = await res.json();
        throw new Error(err.detail || "Google Admin Login failed");
    }
